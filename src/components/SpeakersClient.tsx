...
export default function SpeakersClient({ lang, dict, speakers }: Props) {
  ...
  const filterMap: Record<string, string[]> = {
    ...
    'Literature': ['Literature', 'Communication', 'Narrative Theology'],
    'الأدب': ['الأدب', 'التواصل', 'اللاهوت السردي'],
  ...
  return (
    <div className="relative overflow-x-hidden bg-[#fafafa] selection:bg-[#F1A91E] selection:text-white">
          ...
          <div className="container-pad">
            ...
            >
              <span className="text-[#F1A91E] font-bold tracking-widest text-xs uppercase">
                {isAr ? 'المتحدثون الرئيسيون' : 'Featured Speakers'}
              </span>
      ...
      {/* ── REST OF TEAM ── */}
      {rest.length > 0 && (
        <section className="py-24 bg-[#06091a] relative overflow-hidden">
          {/* Grid bg */}
          ...
          <div className="container-pad relative z-10">
            ...
            >
              <span className="text-[#F1A91E] font-bold tracking-widest text-xs uppercase">
                {isAr ? 'بقية الفريق' : 'The Full Team'}
              </span>
              <div className="w-8 h-[2px] bg-[#F1A91E] mt-2" />
            </motion.div>
              ...
              {rest.map((s, i) => (
                <motion.div
                  ...
                  style={{
                    ...
                    borderColor: expandedId === s.id ? `${s.color}60` : 'rgba(255,255,255,0.08)',
                  }}
                >
                  <div className="h-1" style={{ backgroundColor: s.color }} />
                  <div className="p-8">
                    <div
                      className="w-14 h-14 rounded-xl flex items-center justify-center text-2xl mb-5 group-hover:scale-110 transition-transform"
                      style={{ backgroundColor: `${s.color}20` }}
                    >
                      {s.image}
                    </div>

                    <h4 className="text-xl font-black text-white mb-1">{isAr ? s.name_ar : s.name}</h4>
                    <p className="text-sm font-bold uppercase tracking-tight mb-4" style={{ color: s.color }}>
                      {isAr ? s.title_ar : s.title}
                    </p>
                    ...
                    <div className="flex flex-wrap gap-1.5 mb-4">
                      {(isAr ? s.expertise_ar : s.expertise).slice(0, 3).map((tag) => (
                        <span
                          key={tag}
                    ...
                    >
                      <div className="pt-4 border-t border-white/10">
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {(isAr ? s.topics_ar : s.topics).map((t) => (
                            <span key={t} className="px-2.5 py-1 rounded-full text-xs text-white/50 border border-white/10">
                              {t}
                            </span>
                          ))}
                        </div>
                        <p className="text-xs italic text-white/25 mt-4">{isAr ? s.credentials_ar : s.credentials}</p>
                      </div>
                    </motion.div>
